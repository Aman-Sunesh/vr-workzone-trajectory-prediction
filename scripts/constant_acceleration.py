...
METHOD_GROUP = "constant_acceleration"

RESULTS_DIR, PLOTS_DIR = make_output_dirs(PROJECT_ROOT, METHOD_GROUP)


def constant_acceleration(type="last_step", num_plots=20):
    ...
    for i in range(len(X_test)):
        ...
        if type == "last_step":
            velocity_step = displacements[-1]
            acceleration_step = displacements[-1] - displacements[-2]

        elif type == "smoothed":
            velocity_step = displacements.mean(dim=0)
            acceleration_steps = displacements[1:] - displacements[:-1]
            acceleration_step = acceleration_steps.mean(dim=0)

        else:
            raise ValueError(f"Unknown constant acceleration type: {type}")
        ...
        if i in plot_positions:
            plot_trajectory_example(
                ...
                y_true=y_true,
                y_pred=y_pred,
                method_name=f"constant_acceleration_{type}",
                idx=original_idx,
                ade=ade,
                fde=fde,
                plots_dir=PLOTS_DIR
    ...
    save_csv(
        os.path.join(RESULTS_DIR, f"constant_acceleration_{type}_per_sample.csv"),
        per_sample_rows,
        fieldnames=["sample_idx", "ade", "fde"]
    )

    print(f"\nConstant Acceleration Type: {type}")
    print("ADE:", mean_ade)
    print("FDE:", mean_fde)

    return {
        "method": f"constant_acceleration_{type}",
        "ade": mean_ade,
        "fde": mean_fde,
...
def main():
    ...
    results.append(constant_acceleration(type="last_step", num_plots=20))
    results.append(constant_acceleration(type="smoothed", num_plots=20))
    ...
    plot_metric_comparison(
        ...
        PLOTS_DIR,
        METHOD_GROUP,
        title="Constant Acceleration Baseline Comparison"
    )
