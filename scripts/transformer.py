...
class TransformerBaseline(nn.Module):
    def __init__(
        ...
        self.head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(d_model, output_steps * 2)
        )
...
def run_transformer(
    ...
    method_name = (
        f"transformer_d{d_model}_heads_{nhead}_"
        f"layers_{num_layers}_ff_{dim_feedforward}_lr_{lr_name}"
    )
...
def main():
    ...
    results.append(run_transformer(d_model=64, nhead=4, num_layers=1, dim_feedforward=128, epochs=100, lr=3e-4, num_plots=20))
    results.append(run_transformer(d_model=128, nhead=4, num_layers=1, dim_feedforward=256, epochs=100, lr=3e-4, num_plots=20))
    results.append(run_transformer(d_model=128, nhead=4, num_layers=2, dim_feedforward=256, epochs=100, lr=3e-4, num_plots=20))
    results.append(run_transformer(d_model=128, nhead=4, num_layers=2, dim_feedforward=512, epochs=100, lr=3e-4, num_plots=20))
    ...
    plot_metric_comparison(
        ...
        PLOTS_DIR,
        METHOD_GROUP,
        title="Transformer Baseline Comparison"
    )
